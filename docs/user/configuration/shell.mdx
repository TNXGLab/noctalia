...
- **Window switcher** is an Alt+Tab-style overlay with no `[shell]` settings yet. Bind your compositor shortcut to `noctalia msg window-switcher` (see [Shell → Window switcher](/noctalia/ipc/shell/#window-switcher)). While open, windows appear in a centered grid (up to five columns) with titles, per-window close controls, and cached content previews on Hyprland; unsupported windows and compositors fall back to application icons. Cached frames appear immediately on the next activation and refresh asynchronously in the background. Tab / Shift+Tab cycle selection; arrow keys move within the grid; releasing Alt or Enter confirms; Escape cancels. The list updates live when windows open, close, or retitle while the overlay stays open. Hyprland users must apply the [dedicated window-switcher layer rule](/noctalia/compositor-settings/hyprland/#window-switcher-previews-and-backdrop).

...
Alt+Tab-style overlay: a dimmed fullscreen layer with a centered grid of open windows (window content preview when supported, title, optional app name). There is no `[shell.window_switcher]` configuration block yet - wire your compositor shortcut to the IPC commands below.
...
On Hyprland, Noctalia captures each tile through `hyprland-toplevel-export-v1`. Preview frames remain cached between activations and refresh in the background, so reopening the switcher does not repopulate tiles one by one. Unsupported or uncapturable windows fall back to their application icon. Hyprland also needs a dedicated backdrop rule; see [Window switcher previews and backdrop](/noctalia/compositor-settings/hyprland/#window-switcher-previews-and-backdrop).

...
namespace {
  constexpr Logger kLog("toplevel-capture");
}

struct ToplevelCapturePending {
  ToplevelCapture* owner = nullptr;
...
namespace {
...
#ifdef __linux__
    const int fd = memfd_create("noctalia-toplevel-capture", MFD_CLOEXEC | MFD_ALLOW_SEALING);
#else
    const int fd = -1;
#endif
    if (fd < 0) {
  ...
  const wl_buffer_listener kBufferListener = {
      .release =
          [](void* data, wl_buffer* /*buffer*/) {
            auto* pending = static_cast<ToplevelCapturePending*>(data);
            if (pending == nullptr) {
              return;
            }
  ...
  void tryIssueCopy(ToplevelCapturePending* pending) {
    ...
    hyprland_toplevel_export_frame_v1_copy(pending->frame, pending->buffer, 0);
    wl_display_flush(pending->owner->wayland().display());
  }

  // Event order follows hyprland-toplevel-export-v1.xml: buffer, damage, flags,
  // ready, failed, linux_dmabuf, buffer_done.
  const hyprland_toplevel_export_frame_v1_listener kFrameListener = {
      .buffer =
          [](void* data, hyprland_toplevel_export_frame_v1* /*frame*/, std::uint32_t format, std::uint32_t width,
      ...
      // copy() without ignore_damage; the switcher captures one-shot frames.
      .damage =
          [](void* /*data*/, hyprland_toplevel_export_frame_v1* /*frame*/, std::uint32_t /*x*/, std::uint32_t /*y*/,
             std::uint32_t /*width*/, std::uint32_t /*height*/) {},
      .flags =
          [](void* data, hyprland_toplevel_export_frame_v1* /*frame*/, std::uint32_t flags) {
            auto* pending = static_cast<ToplevelCapturePending*>(data);
            if (pending == nullptr) {
              return;
            }
            pending->yInvert = (flags & HYPRLAND_TOPLEVEL_EXPORT_FRAME_V1_FLAGS_Y_INVERT) != 0;
          },
      .ready =
          [](void* data, hyprland_toplevel_export_frame_v1* frame, std::uint32_t /*tvSecHi*/, std::uint32_t /*tvSecLo*/,
             std::uint32_t /*tvNsec*/) {
            auto* pending = static_cast<ToplevelCapturePending*>(data);
            ...
            if (!convertToRgba(
              ...
              destroyShmBuffer(pending->pool, pending->buffer, pending->mapped, pending->mappedSize, pending->fd);
              pending->owner->fail(
                  "unsupported toplevel capture buffer layout (format=0x"
      ...
      .failed =
          [](void* data, hyprland_toplevel_export_frame_v1* frame) {
            auto* pending = static_cast<ToplevelCapturePending*>(data);
            hyprland_toplevel_export_frame_v1_destroy(frame);
            if (pending != nullptr) {
              pending->frame = nullptr;
            ...
            if (pending != nullptr && pending->owner != nullptr) {
              pending->owner->fail("toplevel capture frame failed");
            }
          },
      // SHM path only; no linux-dmabuf support needed for switcher previews.
      .linux_dmabuf =
          [](void* /*data*/, hyprland_toplevel_export_frame_v1* /*frame*/, std::uint32_t /*format*/,
             std::uint32_t /*width*/, std::uint32_t /*height*/) {},
      .buffer_done =
          [](void* data, hyprland_toplevel_export_frame_v1* /*frame*/) {
            auto* pending = static_cast<ToplevelCapturePending*>(data);
...
ToplevelCapture::ToplevelCapture(WaylandConnection& wayland) : m_wayland(wayland) {}

ToplevelCapture::~ToplevelCapture() { destroyPending(); }

bool ToplevelCapture::available() const noexcept
{
  return m_wayland.hasToplevelExport() && m_wayland.shm() != nullptr;
}

void ToplevelCapture::capture(
    zwlr_foreign_toplevel_handle_v1* handle, bool overlayCursor, CompletionCallback onComplete
) {
  if (m_busy) {
    if (onComplete) {
  ...
  if (!available() || handle == nullptr) {
    if (onComplete) {
      onComplete(std::nullopt, "toplevel capture unavailable");
    }
    return;
  }
  ...
  auto* manager = m_wayland.toplevelExportManager();
  if (manager == nullptr) {
    if (onComplete) {
      onComplete(std::nullopt, "toplevel export manager missing");
    }
    return;
  }
  ...
  m_pending = std::make_unique<ToplevelCapturePending>();
  m_pending->owner = this;

  // v2 request: takes the zwlr_foreign_toplevel_handle_v1 the switcher already
  // holds for every entry, avoiding the window-address mapping path.
  m_pending->frame = hyprland_toplevel_export_manager_v1_capture_toplevel_with_wlr_toplevel_handle(
      manager, overlayCursor ? 1 : 0, handle
  );

  hyprland_toplevel_export_frame_v1_add_listener(m_pending->frame, &kFrameListener, m_pending.get());
  wl_display_flush(wayland().display());
}

void ToplevelCapture::cancelInFlight() {
  destroyPending();
  m_busy = false;
  m_onComplete = nullptr;
}

void ToplevelCapture::fail(std::string message) {
  destroyPending();
  m_busy = false;
  if (m_onComplete) {
    auto callback = std::move(m_onComplete);
    m_onComplete = nullptr;
    callback(std::nullopt, std::move(message));
  }
}

void ToplevelCapture::finish(ScreencopyImage image) {
  destroyPending();
  m_busy = false;
  if (m_onComplete) {
    auto callback = std::move(m_onComplete);
...
void ToplevelCapture::destroyPending() {
  if (m_pending == nullptr) {
    return;
  }
  if (m_pending->frame != nullptr) {
    hyprland_toplevel_export_frame_v1_destroy(m_pending->frame);
